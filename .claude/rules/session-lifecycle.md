...
1. `bd prime` — reload workflow context and memories
2. `bd show --current` — check for in-progress work from a prior session
3. `bd ready` — find the next unblocked task
...
**Do NOT use TodoWrite, CreatePlan, TaskCreate, markdown checklists, or any IDE-native task tool for tracking work.** These tools do not persist across sessions and are invisible to other agents.

Use `bd create` to track tasks. Use `bd close` to mark them done. Beads is the single source of truth.

The ONLY acceptable use of TodoWrite is for ephemeral within-turn scratch organization that will be captured in beads before the turn ends.
